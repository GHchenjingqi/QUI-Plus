...
if ( ! function_exists( 'csf_customize_validate_url' ) ) {
  function csf_customize_validate_url( $validity, $value, $wp_customize ) {
	...
	/*validate code*/
if ( ! function_exists( 'csf_customize_validate_active_code' ) ) {
	function csf_customize_validate_active_code( $value ) {
		global $code_status,$codes;
		$validity = "激活码不正确";
		if(strlen($value) != 19 ){
			return esc_html__( $validity, 'csf' );
		}
  	}
}

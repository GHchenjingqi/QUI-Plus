<?php
if (!defined('QUI_PLUGIN_URL'))
	define('QUI_PLUGIN_URL', untrailingslashit(plugins_url('', __FILE__)));

	define('QUI_IMAGE_PLACEHOLDER', get_template_directory_uri()."/static/img/default_spcial.jpg");
...
function editTaxonomyField($taxonomy) {
	...
	if (getTaxonomyImageUrl($taxonomy->term_id, NULL, TRUE ) == QUI_IMAGE_PLACEHOLDER)
		$image_text = "";
	else
	...
	return '<script type="text/javascript">
	    jQuery(document).ready(function($) {
			...
			$(".q_remove_image_button").click(function() {
				$("#taxonomy_image").val("");
				$(this).parent().siblings(".title").children("img").attr("src","' . QUI_IMAGE_PLACEHOLDER . '");
			...
			$(".editinline").live("click", function(){
				...
				if (thumb != "' . QUI_IMAGE_PLACEHOLDER . '") {
...
function getTaxonomyImageUrl($term_id = NULL, $size = NULL, $return_placeholder = FALSE) {
	...
	return ($taxonomy_image_url != '') ? $taxonomy_image_url : QUI_IMAGE_PLACEHOLDER;
}
